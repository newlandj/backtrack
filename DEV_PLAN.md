...
1. **Hold-to-preview UX is not feasible as described; resolved as flash-and-fade instead.** The "hold modifier, tap to step through, release to commit" interaction (like OS-level Alt-Tab) needs a keyup event, and `chrome.commands` only fires on keydown — there's no built-in keyup signal when a shortcut is released. v0.1–v0.4 shipped with path (a): each keypress immediately jumps, no visual. v0.5 added a lightweight compromise: after each jump, `chrome.scripting.executeScript` injects `dist/hud.js` into the tab just landed on, which renders a small overlay (via `chrome.tabs.sendMessage`) that fades out after ~1.4s of inactivity — giving positional feedback without needing the keyup signal. This required adding `scripting` + `host_permissions: ["<all_urls>"]` (the target tab isn't the one the keypress originated on, so `activeTab` doesn't cover it), a deliberate tradeoff against the "minimal permission footprint" differentiator called out in `SUMMARY.md` — worth it for the visual feedback, but flagged here since it's exactly the kind of permission bloat the competitive research flagged as a gap to avoid. As predicted, it doesn't render on `chrome://` pages or other restricted schemes (the injection fails silently, caught and ignored) — the tab jump itself is unaffected there, since it never depended on the HUD succeeding.
   ...
   - **True hold/release, revisited.** Path (b) above was deferred as needing "a content script listening for keyup globally, which needs broader host permissions" — but v0.5 already added `scripting` + `host_permissions: ["<all_urls>"]` for the HUD itself, so that blocker was already paid for. `hud.ts` now listens for the physical modifier-key `keyup` (and `blur`, in case focus leaves the browser entirely) directly on the page it's injected into, and dismisses immediately on release rather than waiting out the fallback timer — giving the real hold-to-preview/release-to-dismiss feel for the common case of holding the modifier and tapping the letter repeatedly. `FALLBACK_HOLD_MS` (1.4s) still exists for the case where a quick single tap-and-release beats the injection round-trip (the modifier already released before the listener could be attached).
   - **Modifier changed from Alt to Ctrl, and W dropped in favor of E.** `Alt` sits directly next to `Cmd` on a Mac keyboard, and holding `Alt+Q` to cycle tabs is one slipped keypress away from `Cmd+Q` — which quits the entire browser. `Ctrl` is in the far corner, away from `Cmd`, making that slip much less likely. This also forced dropping `W`: `Ctrl+W` (close tab) is on Chrome's list of shortcuts reserved at the browser-chrome level that extensions cannot bind to at all, the same reservation that already ruled out `Ctrl+Tab` above — go-forward uses `E` instead, keeping both shortcuts on the same QWERTY row for a comfortable one-hand hold-and-tap. `hud.ts`'s keyup listener was updated to watch for `Control` instead of `Alt`.
   - **Switched again, to Alt+Left/Alt+Right.** `Ctrl+Q`/`Ctrl+E` turned out to be an awkward left-hand reach; arrow keys are comfortably reachable with the right hand instead. This reintroduces `Alt` as the modifier, but without the `Cmd+Q`-quit risk that ruled it out for letter keys — Mac Chrome's own page-history shortcut is `Cmd+[`/`Cmd+]`, not `Cmd+Left`/`Cmd+Right`, so there's no destructive action a slipped keypress could land on here. Left/right also reads naturally as back/forward. `hud.ts`'s keyup listener went back to watching for `Alt`.
   - **Made optional.** An on/off toggle (`hudEnabled`, default on) was added to the options page for informal A/B testing with a few people to see whether the HUD is actually wanted or just noise. Stored in `chrome.storage.local` alongside `scopeMode` (same rationale: it's a standing preference, not per-session stack data). No new permissions — `showHud()` in `background.ts` just short-circuits before the `chrome.scripting.executeScript` call when disabled, so the underlying `scripting`/`host_permissions` grant still needs to be accepted at install time even if the HUD ends up toggled off.
